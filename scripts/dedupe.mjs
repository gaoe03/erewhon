...
export function classify(cand, archive) {
  ...
  const sameName = archive.find((s) => normName(s.name) === normName(cand.name));
  const byPid = archive.filter((s) => s.productId && s.productId === cand.productId);
  if (byPid.length) {
    const sameEdition = byPid.find((s) => normName(s.name) === normName(cand.name));
    if (sameEdition) return { action: 'still-live', matchId: sameEdition.id };
    if (sameName) return { action: 'rename', matchId: sameName.id, reusedFrom: byPid[0].id };
  ...
  if (sameName) return { action: 'rename', matchId: sameName.id };
